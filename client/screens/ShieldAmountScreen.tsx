...
import { createShieldTransaction, solToLamports } from "@/lib/shield-transaction";

type NavigationProp = NativeStackNavigationProp<RootStackParamList, "ShieldAmount">;
type RouteProps = RouteProp<RootStackParamList, "ShieldAmount">;

export default function ShieldAmountScreen() {
  ...
  const fetchBalance = async () => {
    try {
      ...
      const balanceInfo = await getBalance(pubkey);
      setMaxBalance(balanceInfo.sol);
    } catch (error) {
  ...
  const handleContinue = useCallback(async () => {
    ...
    try {
      ...
      const result = await createShieldTransaction(pubkey, lamports);
      ...
      console.error("Failed to create shield tx:", error);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        <Text style={styles.title}>SHIELD BALANCE</Text>
        <Text style={styles.subtitle}>
          Convert public SOL to private (compressed) balance
        </Text>

        <View style={styles.balanceInfo}>
          <Text style={styles.balanceLabel}>Available to shield</Text>
          {isLoading ? (
